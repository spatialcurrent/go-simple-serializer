...
GOTAGS= CGO_ENABLED=1 GOOS=linux GOARCH=amd64 go build --tags "linux" -o "gss_linux_amd64" -ldflags "$LDFLAGS" github.com/spatialcurrent/go-simple-serializer/cmd/gss
if [[ "$?" != 0 ]] ; then
...
GOTAGS= CGO_ENABLED=1 GOOS=windows GOARCH=amd64 CXX=x86_64-w64-mingw32-g++ CC=x86_64-w64-mingw32-gcc go build -o "gss_windows_amd64.exe" -ldflags "$LDFLAGS" github.com/spatialcurrent/go-simple-serializer/cmd/gss
if [[ "$?" != 0 ]] ; then
